...
int x,y;
 for(x=1;x<=78;x++){
 gotoxy(x,2); textcolor(7);cprintf("\xB1");
 gotoxy(x,24); textcolor(7);cprintf("\xB1");
 gotoxy(x,18); textcolor(7);cprintf("\xfe");
 }
 for(y=2;y<=24;y++){
 gotoxy(1,y); textcolor(7);cprintf("\xB1");
 gotoxy(79,y); textcolor(7);cprintf("\xB1");
 }
 textcolor(WHITE);
}
...
void signup(){
	...
	border();
	gotoxy(18,7);
	textcolor(GREEN);cprintf("SIGN UP");
	gotoxy(18,11);
...
void login(){
    ...
    border();
    gotoxy(18,7);
    textcolor(LIGHTGREEN);cprintf("LOG IN");
    gotoxy(18,11);
    printf("Username: ");
    scanf("%s", username_checker);
    gotoxy(18,12);
...
void menu(){
	...
	gotoxy(22,20); textcolor(WHITE); cprintf("\xae \x9 Press ESC to go EXIT & LOG OUT \x9 \xaf");
...
void option_prompt(){
    ...
    gotoxy(35,12);textcolor(LIGHTGREEN);cprintf("\x9 Press any key to NEXT STAGE.");
    gotoxy(35,13);textcolor(7);cprintf("\x9 Press [B] to go back to MENU.");
    gotoxy(35,14);textcolor(WHITE);cprintf("\x9 Press ESC to EXIT & LOG OUT.");
    MW_OPTION = getch();
    MW_OPTION = toupper(MW_OPTION);
